       ...
       :  hash_(hash), kequal_(kequal), prober_(prober), resizeAlpha_(resizeAlpha)
{
    // Initialize any other data members as necessary
		size_=0; //0 items
		loadSize_=0;
    totalProbes_=0; //0 probes
		
    mIndex_=0; //start with table size 11
		std::vector<HashItem*> table(CAPACITIES[mIndex_]); //initialize vector
		table_=table;
}
...
{
	//resize?
	double load = double(loadSize_)/table_.size();
	...
	if(h==npos){ //invalid
		throw std::logic_error("No free location (insert)");
	} 
	else if(table_[h]){ //update value
...
{
	mIndex_++; //new table size
	if(mIndex_>=28){
		throw std::logic_error("Reached max table size");
	}

	std::vector<HashItem*> oldTable=table_;
	std::vector<HashItem*> newTable(CAPACITIES[mIndex_]);
	table_=newTable;

	for(HashItem* hItem: oldTable){
		if(hItem!=nullptr){
			if(hItem->deleted==true){
				delete hItem;
				loadSize_--;
			}
			else{ //rehash
				HASH_INDEX_T h = this->probe(hItem->item.first);
				table_[h]=hItem;
			}
		}
	}
